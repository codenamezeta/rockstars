    ...
    >
      <div className='container flex flex-col items-start xl:flex-row pt-20'>
        <div className='z-10 lg:w-3/4 flex flex-col mt-24 space-y-6 self-center xl:self-auto'>
          ...
          <h1 className='themed-text-1 text-4xl md:text-5xl lg:text-7xl xl:text-7xl font-soleil uppercase text-accent tracking-tight font-bold'>
            {title}
          </h1>
          <h2
            className='mt-3 font-arvo font-bold text-md md:text-lg lg:text-2xl xl:text-xl text-white tracking-tight'
            // style={{ textShadow: '1px -1px hsl(36, 25%, 50%)' }}
          >
            {subtitle}
          </h2>
          <p className='my-4 text-sm md:text-lg lg:text-xl xl:text-lg font-soleil tracking-tight'>
            {text}
          </p>
          {list && (
            <ul className='tracking-tight font-arvo space-y-1 md:space-y-2'>
              {list.map((item, index) => (
                <li
                  key={index}
                  className='text-xs md:text-base lg:text-lg text-foreground flex items-center'
                >
          ...
          <Link href={ctaLink}>
            <Button
              className='lg:text-xl tracking-widest uppercase text-primary-foreground lg:py-8 mt-2 lg:mt-8'
              size='full'
            >
